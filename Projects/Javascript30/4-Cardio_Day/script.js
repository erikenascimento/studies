...
console.table(
	inventors.sort((first, second) => (first.year > second.year ? 1 : -1))
);

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
console.log(
	"\nAll inventors lived for",
	inventors.reduce(
...
console.table(
	inventors.sort((current, next) => {
		current.passed - current.year > next.passed - next.year ? 1 : -1;
	})
);
...
console.log(
	...
	people.sort((current, next) => {
		...
		const [nextLastName, nextFirstName] = current.split(", ");
...
console.log(
	"\nSumed up instances: ",
	data.reduce((object, instance) => {
		if (!object[instance]) object[instance] = 0;
		object[instance]++;
		return object;
	}, {})
);
